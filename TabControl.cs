...
namespace AVDClient
{
    ...
    public class TabControl : System.Windows.Forms.TabControl
    {
        ...
        protected override void OnPaint(PaintEventArgs e)
        {
            ...
            SolidBrush PaintBrush = new SolidBrush(this.BackColor);
            int width = r.Width;
            r = GetTabRect(0);
            r.Width = width;
            e.Graphics.FillRectangle(PaintBrush, r);
//            ControlPaint.DrawBorder(e.Graphics, r, Color.Red, ButtonBorderStyle.None);
            //Draw the Tabs
            for (int index = 0; index <= TabCount - 1; index++)
            {
                ...
                if (index == SelectedIndex) bs = ButtonBorderStyle.None;
                PaintBrush.Color = this.BackColor;
                e.Graphics.FillRectangle(PaintBrush, r);
                ControlPaint.DrawBorder(e.Graphics, r, PaintBrush.Color, bs);
                if (index == SelectedIndex)
                    PaintBrush.Color = Color.White;
                else
                    PaintBrush.Color = Color.FromArgb(0xFC, 0xD4, 0x04);
